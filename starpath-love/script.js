...
function initDevSkip() {
    const devBtn = document.getElementById('dev-skip-btn');
    if (!devBtn) return;
    
    devBtn.addEventListener('click', () => {
        if (screens.constellation.classList.contains('active')) {
            const stars = document.querySelectorAll('.star-node');
            const linesGroup = document.getElementById('lines-group');
            
            linesGroup.innerHTML = '';
            stars.forEach(s => {
                s.classList.remove('available', 'connected');
            });
            ...
            starDataArray.forEach(s => s.element.classList.add('connected'));
            
            for (let i = 1; i < starDataArray.length; i++) {
                const line = document.createElementNS('http://www.w3.org/2000/svg', 'line');
                line.setAttribute('x1', starDataArray[i-1].cx);
                line.setAttribute('y1', starDataArray[i-1].cy);
                line.setAttribute('x2', starDataArray[i].cx);
                line.setAttribute('y2', starDataArray[i].cy);
                line.classList.add('constellation-line');
            ...
            const closingLine = document.createElementNS('http://www.w3.org/2000/svg', 'line');
            closingLine.setAttribute('x1', starDataArray[11].cx);
            closingLine.setAttribute('y1', starDataArray[11].cy);
            closingLine.setAttribute('x2', starDataArray[0].cx);
            closingLine.setAttribute('y2', starDataArray[0].cy);
            closingLine.classList.add('constellation-line');
            closingLine.style.animationDuration = '0.3s';
            linesGroup.appendChild(closingLine);
            
            document.getElementById('connected-count').textContent = '12';
            document.getElementById('constellation-progress').style.width = '100%';
        ...
        if (screens.portal.classList.contains('active')) {
            if (countdownInterval) clearInterval(countdownInterval);
            openPortal();
        }
    });
